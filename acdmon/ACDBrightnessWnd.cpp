...
CACDBrightnessWnd::SetOpacity (int opacity) {
	...
	m_pLowerWnd->SetLayeredWindowAttributes (0, (255 * m_nOpacity * ACD_OSD_ALPHA) / (100 * 100), LWA_ALPHA);
}
...
CACDBrightnessWnd::OnPaint ()
{
	...
	if (m_bPos == ACD_OSD_WND_FOREGROUND) {
		for (UINT i = 0; i <= m_nBrightness; ++i)
			dc.FillSolidRect(ACD_OSD_STEP_LEFT + ACD_OSD_STEP_CX * i, ACD_OSD_STEP_TOP, 12, 15, RGB (255, 255, 255));
	} else {
		CDC dcMemory;
		...
		for (UINT i = 0; i <= m_nBrightness; ++i)
			dc.BitBlt(ACD_OSD_STEP_LEFT + ACD_OSD_STEP_CX * i, ACD_OSD_STEP_TOP, 15, 20, &dcMemory, 0, 0, SRCCOPY);
...
CACDBrightnessWnd::OnInitHotKeys (WPARAM wParam, LPARAM lParam) {
	...
	if (dwIncreaseHotKey == 0)
		...
		::RegisterHotKey (m_hWnd, ACD_BRIGHTNESS_UP_HOTKEY, wModifiers, wVirtualKeyCode);
	...
	if (dwIncreaseHotKey == 0)
		...
		::RegisterHotKey (m_hWnd, ACD_BRIGHTNESS_DOWN_HOTKEY, wModifiers, wVirtualKeyCode);
	...
	switch (nEventType) {
		...
		case DBT_DEVICEREMOVECOMPLETE:
			if (pdb->dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE)
				if (theApp.OnDeviceChange(nEventType, ((PDEV_BROADCAST_DEVICEINTERFACE)pdb)->dbcc_name))
					return TRUE;
		default:
