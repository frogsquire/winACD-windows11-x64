...
ACD_AddDevice (IN PDRIVER_OBJECT DriverObject,
	...
	PDEVICE_EXTENSION deviceExt = (PDEVICE_EXTENSION) filterDeviceObject->DeviceExtension;
	...
	PDEVICE_OBJECT lowerDeviceObject = IoAttachDeviceToDeviceStack(filterDeviceObject, PhysicalDeviceObject);
	...
	filterDeviceObject->AlignmentRequirement = lowerDeviceObject->AlignmentRequirement;
...
ACD_DispatchIoctl (IN PDEVICE_OBJECT FilterDeviceObject, IN PIRP Irp) {
	...
	ULONG cCode = irpStack->Parameters.DeviceIoControl.IoControlCode;

	if (cCode == IOCTL_INTERNAL_USB_SUBMIT_URB) {
		PURB urb = (PURB) irpStack->Parameters.Others.Argument1;
		struct _URB_CONTROL_DESCRIPTOR_REQUEST* desc = (struct _URB_CONTROL_DESCRIPTOR_REQUEST *) urb;
...
ACD_DispatchPnP (IN PDEVICE_OBJECT FilterDeviceObject, IN PIRP Irp) {
	...
	if (minor == IRP_MN_START_DEVICE) {
		...
		IoSetCompletionRoutine(Irp, ACD_IoCallDriverCompletion, &event, TRUE, TRUE, TRUE);
...
ACD_CacheStringDescriptors (IN OUT PDEVICE_EXTENSION DeviceExt) {
	...
	for (index = 1; index <= ACD_LAST_DEVICE_STRING_DESCRIPTOR; ++index) {
		...
		status = ACD_GetStringDescriptor(DeviceExt->lowerDeviceObject, index, desc, desc0->bLength);
		if (!NT_SUCCESS (status)) {
			ExFreePool (desc0);
			ExFreePool (desc);
			return status;
	...
	/* get StringDesc0 */
	status = ACD_GetStringDescriptor(DeviceExt->lowerDeviceObject, 0, desc0, sizeof(USB_STRING_DESCRIPTOR));
	if (!NT_SUCCESS (status)) {
		ExFreePool (desc0);
		return status;
	}
...
ACD_GetStringDescriptor(IN PDEVICE_OBJECT LowerDeviceObject,
	...
	PURB urb = ExAllocatePool(NonPagedPool, sizeof(struct _URB_CONTROL_DESCRIPTOR_REQUEST));
